...
export async function setupFrontend(config: ProjectConfig): Promise<void> {
  ...
  const templateDir = path.join(process.cwd(), 'templates', 'basic-polkadot-dapp');
...
async function copyFrontendTemplate(templateDir: string, frontendDir: string, config: ProjectConfig): Promise<void> {
  await fs.copy(templateDir, frontendDir, {
    filter: (src) => {
      const basename = path.basename(src);
      return !['node_modules', '.git', 'dist', '.DS_Store', 'tsconfig.tsbuildinfo', 'package-lock.json'].includes(basename);
    }
  });
...
async function createBasicFrontendStructure(frontendDir: string, config: ProjectConfig): Promise<void> {
  const packageJson = {
...
async function createBasicReactApp(frontendDir: string, config: ProjectConfig): Promise<void> {
...
async function updateAppTsx(frontendDir: string, config: ProjectConfig): Promise<void> {
