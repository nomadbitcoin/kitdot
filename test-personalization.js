...
async function testPersonalization() {
  ...
  try {
    ...
    const contractsPackageJson = path.join(tempDir, 'contracts', 'package.json');
    ...
    if (!contractsExists) {
      throw new Error('Contracts package.json not found after template loading');
    }
    ...
    if (frontendPkg.name !== expectedFrontendName) {
      throw new Error(`Frontend name not personalized. Expected: ${expectedFrontendName}, Got: ${frontendPkg.name}`);
    }

    if (contractsPkg.name !== expectedContractsName) {
      throw new Error(`Contracts name not personalized. Expected: ${expectedContractsName}, Got: ${contractsPkg.name}`);
    }
