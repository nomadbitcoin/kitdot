...
async function testDefaultTemplateIntegration() {
  ...
  try {
    ...
    const templatePath = path.join(__dirname, TEMPLATE_REGISTRY.default.source.localPath);
    ...
    const contractsPackageJson = path.join(templatePath, 'contracts', 'package.json');

    const frontendExists = await fs.access(frontendPackageJson).then(() => true).catch(() => false);
    const contractsExists = await fs.access(contractsPackageJson).then(() => true).catch(() => false);

    if (!frontendExists) {
      throw new Error(`Frontend package.json not found: ${frontendPackageJson}`);
    }
    if (!contractsExists) {
      throw new Error(`Contracts package.json not found: ${contractsPackageJson}`);
    }

    console.log('✅ Required package.json files exist');

    // Test 4: Check package.json content for personalization targets
    const frontendPkg = JSON.parse(await fs.readFile(frontendPackageJson, 'utf8'));
    const contractsPkg = JSON.parse(await fs.readFile(contractsPackageJson, 'utf8'));
    ...
    const { createTemplateLoader } = await import('./dist/utils/template-loader.js');
    ...
    console.log('3. Check that package.json files are personalized with "my-test-app"');
